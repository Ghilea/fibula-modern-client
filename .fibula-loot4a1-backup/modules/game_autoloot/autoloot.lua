...
local function beginLoot(source, retry)
    ...
    if not source or not isGroundContainer(source) then
        return
    end
...
local function onContainerOpen(container)
    ...
    if isGroundContainer(container) then
        -- This is intentionally interaction-triggered only:
        -- normal right-click/use opens the corpse/container first,
        -- then AutoLoot reacts to that Container.onOpen event.
