...
local originalGameMapMouseRelease
local hookedGameMapMouseRelease
local armedSourceId
...
local function installGameMapRightClickHook()
    if not UIGameMap or not UIGameMap.onMouseRelease or originalGameMapMouseRelease then
        return
    end

    originalGameMapMouseRelease = UIGameMap.onMouseRelease

    hookedGameMapMouseRelease = function(self, mousePosition, mouseButton)
        if AutoLoot.settings.enabled and
           mouseButton == MouseRightButton and
           self.allowNextRelease then

            local tile = self:getTile(mousePosition)
            if tile then
                local useThing = tile:getTopUseThing()

                if useThing and useThing:isItem() and useThing:isContainer() then
                    armSource(useThing)

                    -- Explicitly use the container on right-click. This avoids
                    -- relying on the client's classic/modern mouse-control
                    -- branch before our autoloot can react.
                    g_game.use(useThing)
        ...
        return originalGameMapMouseRelease(self, mousePosition, mouseButton)
    end

    UIGameMap.onMouseRelease = hookedGameMapMouseRelease
end

local function uninstallGameMapRightClickHook()
    if originalGameMapMouseRelease and
       UIGameMap and
       UIGameMap.onMouseRelease == hookedGameMapMouseRelease then
        UIGameMap.onMouseRelease = originalGameMapMouseRelease
    end

    originalGameMapMouseRelease = nil
    hookedGameMapMouseRelease = nil
    clearArmedSource()
end
...
local function isEquippedBackpackContainer(container, backpack)
    ...
    return pos and pos.x == 65535 and pos.y == InventorySlotBack
end
...
local function findOpenBackpack(backpack)
    ...
    for _, container in pairs(g_game.getContainers()) do
        if isEquippedBackpackContainer(container, backpack) then
            return container
        end
    end
...
local function finishLoot(source, target, message)
    ...
    if openedTargetAutomatically and target then
        pcall(function()
            g_game.close(target)
        end)
    end
...
local function beginLoot(source, retry)
    ...
    if not target then
        ...
        if retry >= 5 then
            openedTargetAutomatically = false
            pendingSource = nil
            setStatus('Could not open equipped backpack')
            return
        end
    ...
    if openedTargetAutomatically and target.window then
        -- Keep auto-opened backpack from visually popping up during the loot pass.
        target.window:hide()
    end
...
function AutoLoot.terminate()
    ...
    uninstallGameMapRightClickHook()
