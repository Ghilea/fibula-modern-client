...
local function findOpenBackpack(backpack)
    ...
    -- Prefer a root container with the same item id as the equipped backpack.
    for _, container in pairs(g_game.getContainers()) do
        if isEquippedBackpackContainer(container, backpack) and not container:hasParent() then
            return container
        end
    end

    -- Fallback for servers that mark the opened inventory container as having
    -- a parent even though it originated from the equipment slot.
    for _, container in pairs(g_game.getContainers()) do
...
local function onContainerOpen(container)
    ...
    if pendingSource then
        ...
        if backpack and isEquippedBackpackContainer(container, backpack) then
            g_logger.info(string.format(
                '[Fibula AutoLoot] equipped backpack opened: cid=%d item=%d parent=%s',
                container:getId(),
