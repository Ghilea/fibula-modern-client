       ...
       pendingNestedOpen.runId ==
        ...
        if previousMatches or
           (container:hasParent() and
            ...
            g_logger.info(string.format(
                '[Fibula AutoLoot] nested bag opened: cid=%d item=%d depth=%d previousCid=%s',
                container:getId(),
...
local function onContainerClose(container)
    if ignoreClose[container] then
        ...
        g_logger.info(string.format(
            '[Fibula AutoLoot] ignored expected replacement close: cid=%d',
            container:getId()
        ))
    ...
    if preferredBackpackContainerId ==
       container:getId() then
        preferredBackpackContainerId = nil
    end
    ...
    if frameContainsContainer(container) then
        abortTraversal(
            'Loot stopped: source container closed'
        )
    end
end
