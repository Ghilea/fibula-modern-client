...
local function clampPointToScreen(widget, point)
    if not widget or widget:isDestroyed() or not point then
        return point
    end

    -- Floating windows in the current Fibula UI use absolute widget positions.
    -- Clamp against the actual root widget rather than the immediate parent;
    -- some custom panels have their own geometry/anchors and caused the old
    -- parent-relative clamp to push windows partly off-screen.
    local screen = rootWidget
    if not screen or screen:isDestroyed() then
        return point
    end
    ...
    if not screenPos or not screenSize or not widgetSize then
        return point
    end

    local minX = screenPos.x
    local minY = screenPos.y
    local maxX = screenPos.x + screenSize.width - widgetSize.width
    local maxY = screenPos.y + screenSize.height - widgetSize.height
...
local function clampWidgetToScreen(widget)
    if not widget or widget:isDestroyed() then
        return nil
    end

    local current = widget:getPosition()
    local clamped = clampPointToScreen(widget, current)

    if clamped and
       (clamped.x ~= current.x or clamped.y ~= current.y) then
        widget:setPosition(clamped)

        g_logger.info(string.format(
            '[Fibula Workspace] screen-clamped %s from=%s to=%s',
            tostring(widget:getId()),
            pointtostring(current),
            pointtostring(clamped)
        ))
    end

    return clamped or current
end
...
local function saveWidgetPosition(widget, target)
    ...
    local pos = clampWidgetToScreen(widget)
    if not pos then
        return
    end
...
local function restoreWidgetPosition(widget, target)
    ...
    point = clampPointToScreen(widget, point)
    widget:setPosition(point)

    -- If resolution/root geometry changed, persist the corrected safe point.
    local corrected = pointtostring(point)
    if corrected ~= target.position then
...
local function attachContainerWindow(container)
    ...
    if hasSavedPosition then
        -- Restore immediately, then keep the finite stabilization window below.
        -- We deliberately do NOT hide the window: on Fibula 7.72 an opacity
        -- race could leave the backpack logically open but invisible, which
        -- made the first B press close it and the second B press reopen it.
        ...
        for _, delay in ipairs({
            1, 20, 50, 85, 120, 180, 260, 550, 900, 1400, 2200, 3200, 4200
        }) do
            scheduleEvent(function()
                if not container or
    ...
    if path == 'backpack' and state then
...
local function onContainerOpen(container)
    ...
    -- Attach as early as possible. If game_containers has already created the
    -- window, the saved position is applied immediately in the same event turn.
...
local function onContainerClose(container)
    -- Server/logout teardown also closes containers. Only a close while the
    -- character is still online should change the user's persisted open state.
    if not g_game.isOnline() or not state or not container then
        return
    end
    ...
    if path then
        ...
        for _, savedPath in ipairs(state.openContainerPaths or {}) do
            if savedPath ~= path and
               savedPath:sub(1, #path + 1) ~= path .. '/' then
                table.insert(filtered, savedPath)
            end
        end

        state.openContainerPaths = filtered
        ...
        g_logger.info(
            '[Fibula Workspace] user-closed persistent container: ' .. path
        )
    end
...
local function restoreStaticWindows()
    ...
    local character = getInventoryUi()
    if character then
        clampWidgetToScreen(character)
    end
...
function FibulaWorkspace.init()
    ...
    g_logger.info(
        '[Fibula Workspace] UI 5I.4 stable backpack + screen bounds ready'
    )
end
