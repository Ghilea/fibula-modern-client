...
local function buildCharacterLayout()
    ...
    inventoryUi:setWidth(322)
    inventoryUi:setHeight(268)
    ...
    -- Remove custom remnants from previous Character experiments.
    for _, id in ipairs({
        ...
        'wowCharacterTitle', 'wowGearHint'
    }) do
        destroyChild(panel, id)
    end

    -- Character = gear. Keep combat/chase/PvP controls out of this popup.
    for _, id in ipairs({
    ...
    -- Five slots down each side, WoW-style.
    moveEquipment(panel, 'helmet',   28, 44)
    moveEquipment(panel, 'amulet',   28, 84)
    moveEquipment(panel, 'armor',    28, 124)
    moveEquipment(panel, 'legs',     28, 164)
    moveEquipment(panel, 'boots',    28, 204)

    moveEquipment(panel, 'backpack', 260, 44)
    moveEquipment(panel, 'sword',    260, 84)
    moveEquipment(panel, 'shield',   260, 124)
    moveEquipment(panel, 'ring',     260, 164)
    moveEquipment(panel, 'tools',    260, 204)
    ...
    character.preview:setPosition({ x = 100, y = 64 })
    character.preview:setSize({ width = 122, height = 140 })

    -- Stock Creature style supplies /images/ui/panel_flat. Clear it so only the
    -- player's outfit is visible.
    ...
    character.close:setPosition({ x = 292, y = 7 })
