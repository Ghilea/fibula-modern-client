...
local function buildCharacterLayout()
    ...
    local function frame(id, x, y, w, h, bg, border)
        ...
        widget:setPosition({ x = x, y = y })
        widget:setSize({ width = w, height = h })
        widget:setBackgroundColor(bg or '#0b1420cc')
        widget:setBorderWidth(1)
        widget:setBorderColor(border or '#3f546d')
    ...
    inventoryUi:setWidth(320)
    inventoryUi:setHeight(196)
    ...
    -- Clean out custom remnants from older experiments.
    for _, id in ipairs({
        ...
        'wowPreviewTitle', 'wowCharacterSubTitle', 'wowCombatTitle',
        'wowPvpTitle', 'wowSlotFrame', 'wowControlFrame'
    }) do
        destroyChild(panel, id)
    end

    -- Hide widgets we do not want in this popup.
    for _, id in ipairs({
    ...
    -- Keep the native equipment grid intact; just ensure it is visible.
    for _, id in ipairs({
    ...
    -- No player preview in this version.
    ...
    createLabel(panel, 'wowEquipmentTitle', 'EQUIPMENT', 16, 34, 100, 15, '#7f91a8')
    createLabel(panel, 'wowCombatTitle', 'FIGHTING STYLE', 146, 58, 130, 15, '#7f91a8')
    createLabel(panel, 'wowPvpTitle', 'PVP MODE', 146, 122, 120, 15, '#7f91a8')

    frame('wowSlotFrame', 10, 50, 114, 108, '#0b1420c6', '#40546d')
    frame('wowControlFrame', 138, 50, 168, 108, '#0b1420b8', '#40546d')

    -- Native capacity widget.
    local capacity = panel:getChildById('capacityPanel')
    if capacity then
        place(capacity, 146, 34, 138, 18)
    end

    -- Native combat style buttons.
    local attack = panel:getChildById('attack')
    local balanced = panel:getChildById('balanced')
    local defense = panel:getChildById('defense')
    if attack then place(attack, 148, 82, 24, 24) end
    if balanced then place(balanced, 182, 82, 24, 24) end
    if defense then place(defense, 216, 82, 24, 24) end

    -- Native PvP mode button.
    local pvp = panel:getChildById('pvp')
    if pvp then
        place(pvp, 148, 142, 56, 20)
    end
    ...
    character.close:setPosition({ x = 300, y = 7 })
    character.close:setSize({ width = 16, height = 16 })
