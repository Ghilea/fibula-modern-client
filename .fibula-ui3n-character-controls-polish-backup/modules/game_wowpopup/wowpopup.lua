...
local function buildCharacterLayout()
    ...
    anchorLabel('wowCombatTitle', 'FIGHTING STYLE', 168, 80, 120, 15, '#7f91a8')
    anchorLabel('wowPvpTitle', 'PVP MODE', 168, 154, 90, 15, '#7f91a8')
    ...
    if capacity then
        anchorWidget(capacity, 170, 62, 128, 18)
    end
    ...
    if attack then anchorWidget(attack, 172, 104, 24, 24) end
    if balanced then anchorWidget(balanced, 212, 104, 24, 24) end
    if defense then anchorWidget(defense, 252, 104, 24, 24) end

    -- Native PvP control.
    local pvp = panel:getChildById('pvp')
    if pvp then
        anchorWidget(pvp, 172, 178, 64, 20)
    end
