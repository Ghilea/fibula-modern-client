...
local function buildRow(parent, id, y, labelText, labelColor, borderColor)
    local row = createPanel(parent, id, 0, y, 292, 25)
    row:setBackgroundColor('#071019b8')
    row:setBorderWidth(1)
    row:setBorderColor('#33445a')
...
local function buildUi()
    ...
    holder:setSize({ width = 292, height = 56 })
    ...
    if targetFrame then
        holder:addAnchor(AnchorLeft, 'targetFrame', AnchorLeft)
        holder:addAnchor(AnchorTop, 'targetFrame', AnchorBottom)
        holder:setMarginTop(6)
    else
        holder:addAnchor(AnchorLeft, 'playerFrame', AnchorRight)
        holder:addAnchor(AnchorTop, 'playerFrame', AnchorTop)
        holder:setMarginLeft(14)
        holder:setMarginTop(84)
    end
...
local function applyConditionImage(icon, definition)
    ...
    if definition.path and definition.path ~= '' then
        image:setImageSource(definition.path)
        image:setImageClip('')
    else
        image:setImageSource('/images/game/states/player-state-flags')
        local clip = tonumber(definition.clip) or 1
        local x = math.max(0, clip - 1) * 9
        image:setImageClip(string.format('%d 0 9 9', x))
    end
