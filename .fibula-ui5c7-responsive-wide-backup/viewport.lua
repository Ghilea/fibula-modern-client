...
applyLegacyWalkSync = function()
    -- Fibula legacy server-synced walking:
    -- OpenTibiaBR normally allows local pre-walk prediction. On old 7.72,
    -- sustained prediction can visually advance the followed camera before the
    -- server-confirmed edge row/column has arrived.
    if g_game.getClientVersion() < 780 then
        if g_game.getFeature(GameAllowPreWalk) then
            g_game.disableFeature(GameAllowPreWalk)
        end
        g_logger.info('[Fibula Viewport] legacy pre-walk disabled; server-synced movement active')
    end
end
...
function FibulaViewport.init()
    ...
    g_logger.info('[Fibula Viewport] UI 5C.5 server-synced walk ready')
end
