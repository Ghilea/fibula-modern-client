...
-- Fibula condition asset guard 5D.1
...
local function conditionImagePath(definition)
    ...
    if definition.path and definition.path ~= '' then
        ...
        -- Test both the path exactly as ConditionIcons supplies it and an
        -- absolute resource-root variant. We only use a candidate when the
        -- resource manager confirms that it exists.
    ...
    for _, candidate in ipairs(candidates) do
        if resourceExists(candidate) then
            return candidate
        end
    end
...
local function conditionFallbackMark(icon, definition)
    ...
    local short = text:sub(1, 1):upper()
    mark:setText(short)
    mark:show()
end
...
local function applyConditionImage(icon, definition)
    ...
    local path = conditionImagePath(definition)
    local mark = icon:getChildById('stateFallbackMark')
    ...
    if path then
        ...
        -- Do not call setImageSource() with a missing path: doing so is what
        -- generated the repeated "Unable to load texture" errors. The state
        -- still remains visible and hoverable through this one-letter fallback.
        image:hide()
        conditionFallbackMark(icon, definition)
    end
end
