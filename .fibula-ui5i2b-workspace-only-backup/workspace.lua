...
local function attachContainerWindow(container)
    ...
    -- Several runtime modules polish/position a newly created container window.
    -- Re-apply the saved position after those one-shot layout passes. This is
    -- still event-driven; there is no permanent polling.
    for _, delay in ipairs({ 120, 260, 550, 900, 1300 }) do
        scheduleEvent(function()
    ...
    g_logger.info(string.format(
        '[Fibula Workspace] container attached: cid=%d path=%s name=%s',
        container:getId(),
        path,
...
function FibulaWorkspace.init()
    ...
    g_logger.info(
        '[Fibula Workspace] UI 5I.1 backpack persistence fix ready'
    )
end
