...
local function clampPointToParent(widget, point, margin)
    ...
    if not parentPos or not parentSize or not widgetSize then
        return point
    end

    local minX = parentPos.x + margin
    local minY = parentPos.y + margin
    local maxX = parentPos.x + parentSize.width - widgetSize.width - margin
    local maxY = parentPos.y + parentSize.height - widgetSize.height - margin
...
local function clampWidgetToParent(widget, margin)
    if not widget or widget:isDestroyed() then
        return nil
    end

    local current = widget:getPosition()
    local clamped = clampPointToParent(widget, current, margin)
    ...
    if clamped and
       (clamped.x ~= current.x or clamped.y ~= current.y) then
        ...
        g_logger.info(string.format(
            '[Fibula Workspace] clamped %s from=%s to=%s',
...
local function saveWidgetPosition(widget, target)
    ...
    local pos = clampWidgetToParent(widget, 6)
    if not pos then
        return
    end
...
local function restoreWidgetPosition(widget, target)
    ...
    point = clampPointToParent(widget, point, 6)
...
local function prepareContainerBeforeFirstDraw(container)
    if not state or not container or not container.window or container.window:isDestroyed() then
        return false
    end
    ...
    -- Container.onOpen callbacks are processed before the next rendered frame.
    -- Making the already-created window transparent here prevents the visible
    -- "default position -> saved position" jump.
    if not container.fibulaWorkspacePreHidden then
        container.fibulaWorkspacePreHidden = true
        container.fibulaWorkspaceSavedOpacity = window:getOpacity()
        window:setOpacity(0)
    end

    restoreWidgetPosition(window, containerState)
    return true
end

local function revealPreparedContainer(container)
    if not container or not container.window or container.window:isDestroyed() then
        return
    end
    ...
    container.window:setOpacity(container.fibulaWorkspaceSavedOpacity or 1)
    container.fibulaWorkspacePreHidden = false
    container.fibulaWorkspaceSavedOpacity = nil
end
...
local function attachContainerWindow(container)
    ...
    if hasSavedPosition then
        -- The window was hidden in onContainerOpen before the next rendered
        -- frame. Put it at the saved/clamped point and reveal it immediately.
        restoreContainerPosition(container)
        scheduleEvent(function()
            revealPreparedContainer(container)
        end, 1)
...
local function onContainerOpen(container)
    ...
    -- game_containers has already created container.window in the same
    -- Container.onOpen dispatch. Hide/position it before the next draw so the
    -- user does not see the default cascade flash first.
    prepareContainerBeforeFirstDraw(container)
...
local function restoreStaticWindows()
    ...
    if character then
        clampWidgetToParent(character, 6)
    end
...
function FibulaWorkspace.init()
    ...
    g_logger.info(
        '[Fibula Workspace] UI 5I.3 edge-safe no-flash persistence ready'
    )
end
