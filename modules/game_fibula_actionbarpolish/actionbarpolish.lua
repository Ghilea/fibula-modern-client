...
local function requestRefresh(delay)
    if coalescedRefreshEvent then
        return
    end

    coalescedRefreshEvent = scheduleEvent(function()
        coalescedRefreshEvent = nil
        FibulaActionbarPolish.refresh()
    end, delay or 35)
end

local function onContainerMutation()
    requestRefresh(35)
end

local function onInventoryMutation()
    requestRefresh(35)
end
...
function FibulaActionbarPolish.init()
    ...
    connect(Container, {
        onAddItem = onContainerMutation,
        onUpdateItem = onContainerMutation,
        onRemoveItem = onContainerMutation
    })

    connect(LocalPlayer, {
        onInventoryChange = onInventoryMutation
    })
    ...
    g_logger.info('[Fibula Actionbar] UI 5B.1 performance mode ready')
end
...
function FibulaActionbarPolish.terminate()
    ...
    if coalescedRefreshEvent then
        removeEvent(coalescedRefreshEvent)
        coalescedRefreshEvent = nil
    end
