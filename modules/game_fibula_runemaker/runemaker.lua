...
local originalTalkChannel = nil
local hookedTalkChannel = nil
...
local function looksLikeLegacyRuneSpell(words)
    local normalized = normalizeWords(words)

    -- Classic rune-conjuring words are predominantly in these "ad..." spell
    -- families. This fallback is used only if OTClient's SpellInfo lookup did
    -- not recognize the exact Fibula text.
    local prefixes = {
        'adori ',
        'adura ',
        'adevo ',
...
local function resolveRuneSpell(words)
    local data = findSpellData(words)

    if data then
        return data
    end

    if looksLikeLegacyRuneSpell(words) then
        return {
            name = 'Legacy Rune Conjure',
            type = 'Conjure',
            words = words,
            source = 3147,
            fibulaFallback = true
...
local function rawTalkChannel(mode, channelId, text)
    if originalTalkChannel then
        return originalTalkChannel(mode, channelId, text)
    end
...
local function temporaryContainerDestination(container, label)
    if not containerHasFreeSlot(container) then
        return nil
    end
    ...
    return {
        ...
        position = freeSlotPosition(container),
        label = label or ('container ' .. tostring(container:getId()))
    }
end

local function findTemporaryDestination(sourceItem)
    local sourceParent = getItemParentContainer(sourceItem)

    -- 1. Best case: the same bag as the Blank Rune has a genuinely free slot.
    local destination = temporaryContainerDestination(
        sourceParent,
        'source bag'
    )
    if destination then
    ...
    -- 2. Prefer the equipped root backpack if it is a different container.
    local rootBackpack = getEquippedBackpackRoot()
    if rootBackpack ~= sourceParent then
        destination = temporaryContainerDestination(
            rootBackpack,
            'equipped backpack'
        )
        if destination then
    ...
    -- 3. Scan every OTHER open player-side backpack/bag. This deliberately
    -- excludes ground corpses and ground containers.
    for _, container in pairs(g_game.getContainers()) do
        if container ~= sourceParent and
           container ~= rootBackpack and
           containerIsPlayerLike(container) then
    ...
    -- 4. Classic arrow/torch slot fallback. We only use it if it is EMPTY.
    -- This gives Rune Maker a safe bridge even when every slot in the rune bag
    -- is occupied and no other open backpack has room.
    local player = getPlayer()
    if player and player:getInventoryItem(InventorySlotAmmo) == nil then
        return {
            kind = 'inventory',
            inventorySlot = InventorySlotAmmo,
            position = handPosition(InventorySlotAmmo),
            label = 'ammo/torch slot'
        }
    end
...
local function findMovedHandItem()
    ...
    -- If we used the arrow/torch slot, check that exact slot first so we do
    -- not accidentally restore a different copy of the same item.
    if transaction.tempInventorySlot then
        local player = getPlayer()
        local slotItem = player and
            player:getInventoryItem(transaction.tempInventorySlot) or nil
    ...
    if transaction.tempContainerId then
...
local function castPreparedSpell()
    ...
    -- Bypass our own hooks for the actual prepared cast, preserving the
    -- original outbound route. Plain chat/action text commonly reaches old
    -- OTClient through talkChannel rather than g_game.talk.
    if transaction.sendKind == 'channel' then
        rawTalkChannel(
            transaction.sendMode,
            transaction.sendChannel,
            transaction.words
        )
    else
...
local function freeSelectedHand()
    ...
    local source = findSourceItem(transaction.sourceId)
    local temp = findTemporaryDestination(source)

    if not temp or not temp.position then
        finish(
            'Both hands are occupied and no safe temporary slot is available. Open another backpack/bag or free the arrow/torch slot.',
            true
        )
        return
    end
    ...
    transaction.tempContainerId = temp.containerId
    transaction.tempInventorySlot = temp.inventorySlot

    log(string.format(
        'temporarily moving hand item %d from slot %d to %s',
        transaction.originalHandId,
        transaction.handSlot,
        tostring(temp.label)
    ))

    g_game.move(hand, temp.position, hand:getCount())
...
function castRuneSpell(words, spellData, sendContext)
    ...
    transaction = {
        ...
        tempContainerId = nil,
        tempInventorySlot = nil,
        sendKind = sendContext and sendContext.kind or 'talk',
        sendMode = sendContext and sendContext.mode or nil,
        sendChannel = sendContext and sendContext.channel or nil
    }
...
local function interceptRuneWords(text, sendContext)
    local spellData = resolveRuneSpell(text)
    local sourceId = spellData and tonumber(spellData.source) or 0

    if spellData then
        log(string.format(
            'outgoing spell detected via %s: "%s" spell="%s" type=%s source=%d fallback=%s',
            sendContext and tostring(sendContext.kind) or 'unknown',
            tostring(text),
            tostring(spellData.name or '?'),
            tostring(spellData.type or '?'),
            sourceId,
            tostring(spellData.fibulaFallback == true)
        ))
    end

    if spellData and sourceId > 0 then
        return castRuneSpell(text, spellData, sendContext)
    end

    return false
end

local function installTalkHooks()
    if not originalTalk and g_game and g_game.talk then
        originalTalk = g_game.talk

        hookedTalk = function(text)
            if interceptRuneWords(text, {
                kind = 'talk'
            }) then
                return true
            end
        ...
        log('g_game.talk hook installed')
    end

    if not originalTalkChannel and g_game and g_game.talkChannel then
        originalTalkChannel = g_game.talkChannel

        hookedTalkChannel = function(mode, channelId, text)
            -- game_console.sendMessage() uses talkChannel even for ordinary
            -- local chat in this OTClient. Only recognized rune conjures are
            -- intercepted; every other message passes through unchanged.
            if interceptRuneWords(text, {
                kind = 'channel',
                mode = mode,
                channel = channelId
        ...
        g_game.talkChannel = hookedTalkChannel
        log('g_game.talkChannel hook installed')
    end
end

local function uninstallTalkHooks()
    if originalTalk and g_game and g_game.talk == hookedTalk then
        g_game.talk = originalTalk
    end

    if originalTalkChannel and
       g_game and
       g_game.talkChannel == hookedTalkChannel then
        g_game.talkChannel = originalTalkChannel
    end

    originalTalk = nil
    hookedTalk = nil
    originalTalkChannel = nil
    hookedTalkChannel = nil
end
...
function init()
    ...
    installTalkHooks()
    log('Smart Rune Maker 4E.3 ready')
end
