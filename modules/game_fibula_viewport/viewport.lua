...
-- Fibula viewport forward declaration 5C.6
-- onGameStart() is defined before the implementation below, so the local must
-- exist here for Lua lexical scoping to capture the correct function.
local applyLegacyWalkSync
...
applyLegacyWalkSync = function()
    -- UI 5C.7 responsive walking:
    -- Keep OTClient's normal client-side pre-walk prediction enabled.
    -- This restores the immediate WASD feel. The old 5C.5 server-synced mode
    -- was experimentally useful but did not materially solve the long-walk
    -- edge artifact and felt laggy once 5C.6 made it actually execute.
    if g_game.getClientVersion() < 780 then
        if not g_game.getFeature(GameAllowPreWalk) then
            g_game.enableFeature(GameAllowPreWalk)
        end

        g_logger.info('[Fibula Viewport] legacy pre-walk enabled; responsive movement active')
    end
end
...
function FibulaViewport.init()
    ...
    g_logger.info('[Fibula Viewport] UI 5C.7 responsive wide view ready')
end
